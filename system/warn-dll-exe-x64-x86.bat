...
set /a mpCount=0
for /F "tokens=*" %%a in ('call %~dp0\check-dll-exe-x64-x86.bat %ToCheckDir% --nd "^(obj|target)$" --nf "log4net|Json|Razorvine|PowerArgs" ^| lzmw -S -it "\s*dumpbin.*?header\S*\s+(.+?\.(exe|dll|lib))\s+.*?[\r\n]+\s*(\S+[^\r\n]*machine[^\r\n]*)" -o "$3 : $1\n" -PAC ^| nin nul "machine\s*\(\s*(\w+)\s*\)" -iuw -M') do (
    set /a mpCount+=1
