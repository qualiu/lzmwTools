...
function Update-Kafka-Settings($kafkaDir) {
    ...
    lzmw -it "^(\s*log.dirs)\s*=.*$" -o "`$1=$kafkaLogDirReplace" -p $(Join-Path $kafkaConfigDir "server.properties") -R -c Set kafka log.dirs = $zookeeperDataDir
    
    lzmw -it "^(\s*num.partitions)\s*=.*$" -o "`$1=$KafkaPartitions" -p $(Join-Path $kafkaConfigDir "server.properties") -R -c Set kafka num.partitions = $KafkaPartitions
}
