...
if( -not $(Get-Command lzmw.exe > $null 2>$null) ) {
    if (-not $(Test-Path $(Join-Path $ToolDir lzmw.exe))) {
        Invoke-WebRequest -Uri https://github.com/qualiu/lzmw/blob/master/tools/lzmw.exe?raw=true -OutFile $(Join-Path $ToolDir lzmw.exe)
    }
    
    $env:PATH = $ToolDir + ";" + $env:PATH
}

$ipList = ipconfig | lzmw -it ".*IPv4.*\s+(\d+\.[\d\.]+\d+)\s*$" -o '$1' -PAC
...
function Update-Kafka-Settings($kafkaDir) {
    ...
    # lzmw -it "set\s+LOG_DIR=" -x / -o "\\" -f "\.(bat|cmd)$" -rp $KafkaDirectory  -R -O -c To avoid first time warning
    
    $kafkaConfigDir = Join-Path $kafkaDir "config"
    lzmw -it "^(\s*dataDir)\s*=.*$" -o "`$1=$zookeeperDataDirReplace" -p $(Join-Path $kafkaConfigDir "zookeeper.properties") -R -c Set zookeeper dataDir = $zookeeperDataDir

    $kafkaServerConfigFile = $(Join-Path $kafkaConfigDir "server.properties")
    lzmw -it "^(\s*log.dirs)\s*=.*$" -o "`$1=$kafkaLogDirReplace" -p $kafkaServerConfigFile -R -c Set kafka log.dirs = $zookeeperDataDir
    
    lzmw -it "^(\s*num.partitions)\s*=.*$" -o "`$1=$KafkaPartitions" -p $kafkaServerConfigFile -R -c Set kafka num.partitions = $KafkaPartitions
    
    lzmw -rp $kafkaConfigDir -f properties -x 9092 -o $KafkaPort -R -c Replace kakfa port to $KafkaPort
    lzmw -rp $kafkaConfigDir -f properties -x 2181 -o $ZookeeperPort -R -c Replace zookeeper port to $ZookeeperPort
    lzmw -rp $kafkaConfigDir -f properties -it "^(\s*port)\s*=\s*(\d+)(.*)" -o "`$1=$KafkaPort`$3" -R -c Replace kafka port to $KafkaPort
    if ( $LASTEXITCODE -eq 0 ) {
        lzmw -rp $kafkaConfigDir -f properties -it "^(\s*port)\s*=\s*(\d+)(.*)"
        if ( $LASTEXITCODE -eq 0 ) {
            lzmw -rp $kafkaConfigDir -f properties -it "^(broker.id\s*=.+)" -o "`$0`nport=$KafkaPort" -R -c Add kafka port settings to $KafkaPort
        }
    }
    
    # lzmw -rp $kafkaConfigDir -f properties -it "^(\s*)#?(listeners)=\S+" -o "`$1`$2==PLAINTEXT://192.168.56.1:9292" -R -c Replace zookeeper port.
}
