...
for /f "tokens=*" %%a in ('lzmw -p %KafkaConfigDir% -f "^server-?\d*\.properties$" -it "^\s*broker.id\s*=\s*(\d+).*" -o "$1" -s "\d+" -PAC') do set /a NewestBrokerId=%%a
for /f "tokens=*" %%a in ('lzmw -p %KafkaConfigDir% -f "^server-?\d*\.properties$" -l -PAC ^| lzmw -s "[^\\/]+$" -PAC') do set NewestBrokerFile=%%a

:: Get newest port
for /f "tokens=*" %%a in ('lzmw -p %KafkaConfigDir% -f "^server-?\d*\.properties$" -it "^\s*port\s*=\s*(\d+).*" -o "$1" -s "\d+" -PAC -T 1') do set /a NewestPort=%%a
if "%NewestPort%" == "" (
    for /f "tokens=*" %%a in ('lzmw -p %KafkaConfigDir% -f "^server-?\d*\.properties$" -l -PAC ^| lzmw -s "[^\\/]+$" -PAC') do (
...
for /L %%k in (1,1,%AddCount%) do (
    ...
    lzmw -p !newConfig! -it "^(\s*broker.id)\s*=\s*(\d+)(.*)" -o "$1=!NewestBrokerId!$3" -R -c Update broker.id to !NewestBrokerId!
    lzmw -p !newConfig! -it "^(\s*port)\s*=\s*(\d+)(.*)" -o "$1=!NewestPort!$3" -R -c Update port to !NewestPort!
    lzmw -p !newConfig! -it "^(\s*log.dirs)\s*=\s*(\S+)\d*.*$" -o "$1=${2}-!NewestBrokerId!" -R -c Update log.dirs
)
