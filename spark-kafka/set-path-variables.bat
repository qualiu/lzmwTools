...
for /f "tokens=*" %%a in ('lzmw -z "%~dp0\app" -x \\ -o \ -aPAC') do set "AppRootDir=%%a"

:: Get Hadoop directory
::for /f "tokens=*" %%a in ('dir /S /B /A:D %AppRootDir% ^| lzmw -it "(.*\\Hadoop[^\\]*)\\bin.*$" -o "$1" -PAC') do set "HADOOP_HOME=%%a"
for /f "tokens=*" %%a in ('lzmw -l -rp %AppRootDir% --pp hadoop.*bin -PAC -H 1 -J ^| lzmw -it "(.*\\Hadoop[^\\]*)\\bin.*$" -o "$1" -PAC') do set "HADOOP_HOME=%%a"

:: Get Spark directory
::for /f "tokens=*" %%a in ('dir /S /B /A:D %AppRootDir% ^| lzmw -it "(.*\\Spark[^\\]*)\\bin.*$" -o "$1" -PAC') do set "SPARK_HOME=%%a"
for /f "tokens=*" %%a in ('lzmw -l -rp %AppRootDir% --pp "spark.*bin" -f spark-submit -H 1 -J -PAC ^| lzmw -it "(.*\\Spark[^\\]*)\\bin.*$" -o "$1" -PAC') do set "SPARK_HOME=%%a"

::for /f "tokens=*" %%a in ('dir /S /B /A:D %AppRootDir% ^| lzmw -it "(.*\\Kafka[^\\]*)\\bin\\windows.*$" -o "$1" -PAC') do set "KAFKA_HOME=%%a"
for /f "tokens=*" %%a in ('lzmw -l -rp %AppRootDir% --pp kafka.*bin.*windows -PAC -H 1 -J ^| lzmw -it "(.*\\Kafka[^\\]*)\\bin\\windows.*$" -o "$1" -PAC') do set "KAFKA_HOME=%%a"
...
if "%KAFKA_HOME%" == "" (
    echo Not found Kafka, please check Kafka existence. | lzmw -aPA -t .+
    exit /b -1
)
...
if not "%SkipHadoop%" == "1" (
    if "%HADOOP_HOME%" == "" (
        echo Not found Hadoop, please check Hadoop existence. | lzmw -aPA -t .+
        exit /b -1
    )
...
if not "%SkipSpark%" == "1" (
    if "%SPARK_HOME%" == "" (
        echo Not found Spark, please check Spark existence. | lzmw -aPA -t .+
        exit /b -1
    )
...
::Remove redundant ;
for /f "tokens=*" %%a in ('lzmw -z "%PATH%" -t "\s*;[\s;]*" -o ";" -aPAC') do set "PATH=%%a"

::Remove duplicated pathes in %PATH% to shorten the length of %PATH%
for /f "tokens=*" %%a in ('lzmw -z "%PATH%" -t "\\?\s*;[;\s]*" -o "\n" -PAC ^| nin nul -iuPAC ^| lzmw -S -t "(\S+)[\r\n]+" -o "$1;" -PAC') do set "PATH=%%a"

::Set brokers for some test script files
for /f "tokens=*" %%a in ('lzmw -p %KafkaConfigDir% -f "^server-?\d*\.properties$" -it "^\s*port\s*=\s*(\d+).*" -o "$1" -PAC ^| lzmw -t "\d+" -o "localhost:$0" -PAC ^| lzmw -S -t "\s+(\S+)" -o ",$1" -aPAC') do set "KafkaBrokers=%%a"
...
:: ==========================================================================
:: Will use return value of lzmw.exe at bellow instead of %ERRORLEVEL%
...
:Add_ExeDir_To_PATH
    ...
    lzmw -l -f "\.(exe|bat|cmd)$" -p %1 -PAC >nul
    if !ERRORLEVEL! EQU 0 (
        echo Needless to add to PATH as no exe/bat/cmd found in %1. | lzmw -aPA -e \w+ -x %1
        exit /b 0
    )
    
    :: if %ERRORLEVEL% EQU 0 exit /b 0
    :: Add to %PATH% if directory %1 not in %PATH%
    :: lzmw -z "%PATH%" -ix "%1;" -PAC > nul && set "PATH=%PATH%;%1;"
    
    lzmw -z "%PATH%" -ix "%1;" -PAC > nul
    if !ERRORLEVEL! EQU 0 (
    ...
    lzmw -z "%PATH%" -ix %1 -PA >nul && (echo Failed to add %1 to PATH && exit /b -1)
    
    exit /b 0

:: Input exe-name and directory, check and remove other locations in %PATH%, by where exe
:Clean_ExeDir_In_PATH_Except
    for /f "tokens=*" %%a in ('where %1 2^>nul ^| lzmw -t "\\[^\\]+$" -o "" -aPAC') do (
        if /I "%%a" == "%2" (
            rem Keep, not remove %%a May have 1 redundant at most.
        ) else (
            :: Check and remove %%a; in PATH, if found, not check and remove ;%%a
            lzmw -z "%PATH%" -ix "%%a;" -o "" -PAC >nul || ( ( for /f "tokens=*" %%p in ('lzmw -z "%PATH%" -ix "%%a;" -o "" -aPAC') do set "PATH=%%p" ) & exit /b 0 )
            :: Check and remove ;%%a in PATH
            lzmw -z "%PATH%" -ix ";%%a" -o "" -PAC >nul && for /f "tokens=*" %%p in ('lzmw -z "%PATH%" -ix ";%%a" -o "" -aPAC') do set "PATH=%%p"
        )
    )
    exit /b 0
    
:DisplayAll
    set | lzmw -PA  -it "^(spark\w*|hadoop\w*|kafka\w*|(PATH))=(\S+)" -e "[^;]*(spark|hadoop|kafka|winutils)[^;]*"
    exit /b 0
