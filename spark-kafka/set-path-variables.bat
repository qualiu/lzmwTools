...
for /f "tokens=*" %%a in ('dir /S /B /A:D %~dp0\app ^| lzmw -it "(.*\\Kafka[^\\]*)\\bin\\windows$" -o "$1" -PAC') do set "KAFKA_HOME=%%a"
if "%KAFKA_HOME%" == "" (
...
for /f "tokens=*" %%a in ('dir /S /B /A:D %~dp0\app ^| lzmw -it "(.*\\Hadoop[^\\]*)\\bin$" -o "$1" -PAC') do set "HADOOP_HOME=%%a"
if not "%SkipHadoop%" == "1" (
    if "%HADOOP_HOME%" == "" (
...
:: Get Spark directory
for /f "tokens=*" %%a in ('dir /S /B /A:D %~dp0\app ^| lzmw -it "(.*\\Spark[^\\]*)\\bin$" -o "$1" -PAC') do set "SPARK_HOME=%%a"
if not "%SkipSpark%" == "1" (
    if "%SPARK_HOME%" == "" (
...
:Add_ExeDir_To_PATH
    ...
    lzmw -l -f "\.(exe|bat|cmd)$" -p %1 -PAC >nul && exit /b 0
...
:DisplayAll
    set | lzmw -PA  -it "^(spark\w*|hadoop\w*|kafka\w*)|[^;]*(spark|hadoop|kafka|winutils)[^;]*"
    exit /b 0
